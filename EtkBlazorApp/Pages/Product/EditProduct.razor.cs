...
namespace EtkBlazorApp.Pages.Product
{
    public partial class EditProduct
    {
        ...
        private string TooltipText { get; } = new StringBuilder()
                   .AppendLine("»зменение цены товара/остатка напр€мую (через пол€ выше) - мен€ет его цену/количество только на ближайшее врем€")
                   .AppendLine("Ќа сайте работает автоматический триггер который срабатывает каждые пол часа (и сразу, при загрузке прайс-листа через LK)")
                   .Append("ќн автоматически проставл€ет цену на товар из складов (по определенному алгритму), а остаток расчитывает как сумму всех остатков на всех складах")
                .ToString();
        ...
        private async Task SaveChanges()
        {
            ...
            await log.Write(LogEntryGroupName.ProductUpdate, "“овар обновлен", $"{HttpUtility.HtmlDecode(editedProduct.Name)} ({editedProduct.ProductIdUri})");
            currentStateCode = GetCurrentStateCode();
        }
