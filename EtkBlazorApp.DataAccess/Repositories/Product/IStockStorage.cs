...
namespace EtkBlazorApp.DataAccess.Repositories.Product
{
    public interface IStockStorage
    {
        ...
        Task<List<ProductToStockEntity>> GetStockDataForProducts(IEnumerable<int> productIds);
    ...
    public class StockStorage : IStockStorage
    {
        ...
        public async Task<List<ProductToStockEntity>> GetStockDataForProducts(IEnumerable<int> productIds)
        {
            if (productIds is null || productIds.Count() == 0)
            {
                return new List<ProductToStockEntity>();
            }

            string pidArray = string.Join(",", productIds.OrderBy(i => i));
            string sql = $@"SELECT pts.*, sp.name as stock_name
                           FROM oc_product_to_stock pts
                           JOIN oc_stock_partner sp ON (pts.stock_partner_id = sp.stock_partner_id)
                           WHERE pts.product_id IN({pidArray})";

            var list = await database.GetList<ProductToStockEntity>(sql);
